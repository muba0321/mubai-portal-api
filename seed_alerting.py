...
def seed_data():
    ...
    for m in all_metrics:
        ...
        if not existing:
            ...
            db.session.flush()
            created_metrics[m["name"]] = metric.id
        else:
            created_metrics[m["name"]] = existing.id
    ...
    templates = [
        {
            "name": "系统基础监控",
            "description": "CPU、内存、磁盘、网络、负载等基础系统指标监控",
            "group": "system",
        ...
        {
            "name": "MySQL 监控",
            "description": "MySQL 连接数、QPS、慢查询、主从延迟等指标监控",
            "group": "mysql",
        ...
        {
            "name": "Jenkins 监控",
            "description": "Jenkins 构建状态、队列、执行器等 CI/CD 指标监控",
            "group": "jenkins",
            "metrics": jenkins_metrics,
            "rules": [
        ...
        {
            "name": "Nginx 监控",
            "description": "Nginx 连接数、请求速率、错误率等 Web 服务指标",
            "group": "nginx",
        ...
        {
            "name": "Docker 监控",
            "description": "Docker 容器 CPU、内存、重启等容器运行指标",
            "group": "docker",
    ...
    for t in templates:
        ...
        if not existing:
            template = AlertTemplate(
                ...
                group=t["group"],
                source_type="builtin",
                metric_count=len(t["metrics"]),
                rule_count=len(t["rules"]),
                metrics_def=json.dumps(t["metrics"]),
                rules_def=json.dumps(t["rules"]),
            )
